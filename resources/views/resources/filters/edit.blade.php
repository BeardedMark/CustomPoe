...
@section('title', env('APP_NAME') . ' : Редактирование фильтра предметов ' . $filter->name)
@section('description', 'Редактирование фильтра предметов ' . $filter->name . ' в сообществе ' . env('APP_NAME') . ' для Path of Exile')

@section('content')
    <form action="{{ route('filters.update', $filter) }}" method="POST" enctype="multipart/form-data">
        @method('PUT')
        @csrf
                    ...
                    <div class="col col-12 col-lg">
                        <div class="flex-col-13">
                            <div class="flex-col-8">
                                <h1 class="font-size-h1 decor-gold">Редактирование фильтра предметов</h1>
                    ...
                    <div class="col">
                        <div class="flex-col-21">
                            <div class="row">
                                <div class="col col-12 col-lg">
                                    <div class="flex-col-13">
                                        <h2 class="font-size-h2 decor-gold" for="filter">Содержимое</h2>
                                        <textarea class="decor-textbox code" type="text" id="filter" name="filter" rows="30">{{ old('filter') ?? $filter->filter }}</textarea>
                                    </div>
                                </div>

                                <div class="col col-12 col-lg-5">
                                    <div class="flex-col-13">
                                        <h2 class="font-size-h2 decor-gold" for="filter">Данные</h2>

                                        <div class="flex-col-8">
                                            <div class="flex-col-5">
                                                <label class="font-color-second" for="name">Название</label>
                                                <input class="decor-input" type="text" id="name" name="name"
                                                    required value="{{ old('name') ?? $filter->name }}">
                                            </div>

                                            <div class="flex-col-5">
                                                <label class="font-color-second" for="description">Описание</label>
                                                <textarea class="decor-textbox" type="text" id="description" name="description" rows="3">{{ old('description') ?? $filter->description }}</textarea>
                                            </div>

                                            <div class="flex-col-5">
                                                <label class="font-color-second" for="version">Версия</label>
                                                <input class="decor-input" type="text" id="version" name="version"
                                                    value="{{ old('version') ?? $filter->version }}">
                                            </div>
                                        </div>
                                        ...
                                        <details>
                                            ...
                                            <div class="flex-col-8 mar-t-8">
                                                <div class="flex-col-5">
                                                    <label class="font-color-second" for="type">Тип</label>
                                                ...
                                                <div class="flex-col-5">
                                                    <label class="font-color-second" for="realm">Платформа</label>
                                                    <select class="decor-input" id="realm" name="realm" required>
                                                        @foreach ($filter->realms() as $realm)
                                                            <option value="{{ $realm }}"
                                                                {{ $filter->realm === $realm ? 'selected' : '' }}>
                                                                {{ $realm }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                                ...
                                                <div class="flex-col-5">
                                                    ...
                                                    <textarea class="decor-textbox" type="text" id="content" name="content" rows="10">{{ old('content') ?? $filter->content }}</textarea>
                                                </div>

                                                <div class="flex-col-5">
                                                    <label class="font-color-second" for="comment">Комментарий</label>
                                                    <input class="decor-input" type="text" id="comment" name="comment"
                                                        value="{{ old('comment') ?? $filter->comment }}">
                                                </div>
                                            </div>
                                        </details>
                                            ...
                                            <div class="flex-col-8 mar-t-8">
                                                <div class="flex-col-5">
                                                    ...
                                                    <input class="decor-input" type="text" id="icon"
                                                        name="icon" value="{{ old('icon') ?? $filter->icon }}">
                                                </div>
                                                ...
                                                <div class="flex-col-5">
                                                    ...
                                                    <input class="decor-input" type="text" id="wallpaper"
                                                        name="wallpaper"
                                                        value="{{ old('wallpaper') ?? $filter->wallpaper }}">
                                                </div>

                                                <div class="flex-col-5">
                                                    <label class="font-color-second" for="gallery">Галерея</label>
                                                    <textarea class="decor-textbox" type="text" id="gallery" name="gallery" rows="5">{{ old('gallery') ?? $filter->gallery }}</textarea>
                                                </div>
                                            </div>
                                        </details>
                                                ...
                                                <div class="flex-col-8 mar-t-8">
                                                        ...
                                                        <select class="decor-input" id="user_id" name="user_id" required>
                                                            @foreach ($users as $user)
                                                                <option value="{{ $user->id }}"
                                                                    {{ $filter->user->id === $user->id ? 'selected' : '' }}>
                                                                    {{ $user->name }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                    ...
                                                    <div class="flex-col-5">
                                                        ...
                                                        <input class="decor-input" type="number" id="downloads"
                                                            name="downloads"
                                                            value="{{ old('downloads') ?? $filter->downloads }}">
                                                    </div>
                                                </div>
                                            </details>
                                        @endif
